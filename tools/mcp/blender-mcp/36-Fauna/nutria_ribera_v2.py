# =====================================================================
#  SM_Nutria_Ribera v2 — Generador procedural (Blender 4.x, bpy)
#  Variante ALTA (hero). Estilo cozy: formas orgánicas, low-poly limpio.
#  Mejoras v2: cabeza más grande, hocico definido, bigotes, nariz relieve,
#              patas con volumen, postura curiosa, lomo arqueado.
# =====================================================================
...
def perfil_cuerpo(t):
    ...
    # Lomo más arqueado (curioso = erguido)
    zc = 0.118 + 0.025 * math.sin(math.pi * t)
    y  = CUERPO_Y0 + (CUERPO_Y1 - CUERPO_Y0) * t
    return y, zc, rx, rz

# Cabeza más grande y levantada (postura curiosa)
CABEZA_C = Vector((0.0, 0.215, 0.155))
CABEZA_ATRAS, CABEZA_DELANTE = 0.062, 0.078
...
# Nariz: almohadilla pronunciada
NARIZ_C = None  # se calcula dinámicamente
def perfil_nariz(t):
    """Small bump at the tip of the snout"""
    yn = CABEZA_C.y + CABEZA_DELANTE * 0.98
    zn = CABEZA_C.z - 0.008
    rx_n = 0.018 * (1 - t)
    rz_n = 0.012 * (1 - t)
    y = yn + t * 0.012
    zc = zn + 0.006 * math.sin(math.pi * t)
...
def crear_pata(x, y, trasera):
    ...
    build_tube(bm, rings, None, Vector((x, y + 0.015 * k, 0.0)))
    return bm
...
total = 0
print("\n=== SM_Nutria_Ribera v2 — variante ALTA ===")
for o in TODOS:
