...
Jugador usa herramienta (M13) sobre recurso (M15)
   ...
   â†’ InventoryService.add_item(id, cantidad, BOLSILLO)
        â”œâ”€ pasa si hay stack con espacio â†’ se apila (never se pierde)
        â”œâ”€ si el stack se llena â†’ nuevo slot libre
        â”œâ”€ si no hay stack ni slot libre â†’ signal inventory_full
...
Tecla B / Botón de mochila (M56 controles)
   â†’ InventoryService.pausa_suave(UI only, mundo congelado M29 opcional)
   â†’ InventoryUI.open(bolsillo, contenedor_seleccionado)
   → el jugador selecciona ítem:
        ...
        â”œâ”€ Shift+click â†’ separar stack a la mitad (2 clicks)
        â”œâ”€ R / clic derecho â†’ acciones contextuales (usar, equipar, favorito,
        â”‚                    vender M39, donar M37, descartar M14)
        └─ Tecla de soltar → descarte (con confirmación si es protegido)
   â†’ InventoryService valida y ejecuta
...
M15 (recursos):      recurso.cosechar() â†’ InventoryService.add_item(...)
M16 (crafting):      InventoryService.count_item(id, include_house=true) para materiales;
                     receta.craftear() â†’ remove_item(materiales) â†’ add_item(resultado)
...
| Métrica | Tope |
|---|---|
| Apertura del inventario | â‰¤ 5 ms |
...
| Guardado de todos los contenedores | â‰¤ 10 ms en diferido (M59) |
