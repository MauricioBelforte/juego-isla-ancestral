...
**Modelo:** minimax-m3-free (Kilo Code; corrección 2026-09-01 — el modelo real es **minimax-m3-free en Kilo Code**, NO "MiniMax-M3". Reservas: M35 minimax-m3-free Kilo Code)
...
| M145 Diseño de Experiencia | 🟡 Liberado c/programados | GLM (Kilo) | F0/transversal | V0 | M01 ✅; dueÃ±os M53/M58/M89/M92/M105/M114 | operativa/ con 7 docs (journey, onboarding, menÃºs, feedback, accesibilidad, mÃ©tricas, plan testing); 90/105 + 15 [?] de fase jugable; log 199. 🔵 QA Hy3 2026-08-28: mantiene 🟡, 15 [?] justificados | `DOCUMENTACION/145-Diseno-De-Experiencia/` |
| M146 DiseÃ±o Emocional | 🟡 Liberado c/programados | GLM (Kilo) | F0/transversal | V0 | M145 🟡; M152 ✅ | operativa/ con 5 docs (paleta, mapeo, wow moments, playtesting emocional, cozy checklist); 90/100 + 10 [?] de fase jugable; log 200. 🔵 QA Hy3 2026-08-28: mantiene 🟡, 10 [?] justificados | `DOCUMENTACION/146-Diseno-Emocional/` |
...
| M153 Objetivo Final | 🟡 Liberado c/programados | GLM (Kilo) | F0/gobernanza | V0 | Contrato O1-O19 documentado; dueÃ±os reales | vision_contract.json v1.1 + validate_vision.py (guardiÃ¡n en verde) + prueba_vision.md; 120/130 + 10 [?] telemetrÃ­a/juego; log 202. 🔵 QA Hy3 2026-08-28: mantiene 🟡, 10 [?] justificados | `DOCUMENTACION/153-Objetivo-Final/` |
...
| M19 NPC y Vecinos | 🔵 En curso | glm-5.3-flash (Kilo Code) | F4 | V0/V1 | Base snap+interacciÃ³n F ✅ (auditorÃ­a 2026-08-30); M29 ✅; M20 🟡 nÃºcleo ✅ | Mudanzas (propuesta/aprobaciÃ³n/llegada 08:00 dÃ­a siguiente/partida con enfriamiento 30d) + catÃ¡logo data/villagers/ (5 perfiles) + raycast lÃ­nea de visiÃ³n F + persistencia M59 "npc" + test headless | `game/isla-ancestral/scripts/npc/` (villager_manager.gd, test_mudanzas.gd), `data/villagers/` |
...
| M70 Interacciones (iter. 1) | 🔵 En curso 2026-09-01 | minimax-m3-free (Kilo Code) | F4 (prototipo jugable) | V0/V1 | M11✅ M13✅ | InteractionManager (autoload) orquesta raycast/Ã¡rea + dispatch a IInteractable + prioridad/prompt/cooldown + persistencia M59 + tests headless. Reutiliza `IInteractable.gd` ya existente (scripts/interfaces/). Perfil minimax-m3-free segÃºn guÃ­a 10 Â§6 (tarea agentic larga, complejidad 3, V0) | `game/isla-ancestral/scripts/interacciones/{interaction_manager,interaction_target,interaction_prompt}.gd`, `tests/test_interacciones.gd` (a crear) |
| M30 Reloj en Tiempo Real (iter. 2) | 🔵 En curso 2026-08-31 | glm-5.3 | Cline | F7 (producciÃ³n de contenido) | V0/V1 + V2 nativa | M29 ✅ (GameClock autoload); M53 🟡 (TooltipService operativo) | Hover/desplegable D70 (TooltipService, sin bloquear input), suite bloque E (caso_reloj_tests.gd + caso_reloj.tscn), config w_reloj.tres (F100/F107/F101), scan anti-reloj-SO (C56). Complejidad 2 | `scripts/clock/{w_reloj,w_reloj_config,caso_reloj_tests,preview_reloj}.gd`, `data/ui/w_reloj.tres`, `scenes/caso_reloj.tscn` |
| M17 ConstrucciÃ³n (iter. 1) | 🔵 En curso 2026-09-01 | qwen/qwen3.8-max:free (Kilo Code) | F5 (base de producciÃ³n) | V0 | M08✅ M14🟢 | Sistema de construcciÃ³n voxel complejo: BuildingService autoload + catÃ¡logos data-driven + integraciÃ³n terreno M08 + inventario M14 + economÃ­a M38 + persistencia M59 + tests headless. Perfil Qwen3.8 Max segÃºn guÃ­a 10 Â§8 (complejidad 5, thinking profundo, V0) | `game/isla-ancestral/scripts/construccion/` (a crear), `tests/test_construccion.gd` (a crear) |
...
> ⚠️ **No empezar acÃ¡.** Esta tabla es para verificar orden y dependencias, NO para elegir mÃ³dulo. La fuente de quÃ© mÃ³dulo te corresponde es `CHECKLIST-GLOBAL.md` (columna Recom). Si trabajÃ¡s un mÃ³dulo que no te corresponde, otro modelo que sÃ­ puede hacerlo bien se queda sin tarea.
...
| — | M92 Tutorial | 3 | V0/V2 | 🟡 Liberado — nÃºcleo lÃ³gico | Deepseek V4 Flash | Deepseek V4 Flash (Kilo) | M53 (UI, 🔵 en curso) | TutorialManager autoload (4 capÃ­tulos, triggers de seÃ±al, revalidaciÃ³n, estados, persistencia M59) + test 0 fallos (Log 259). Pendiente: UI burbujas/marcadores (M53, V2), triggers mundo/acciÃ³n, consejos, watchdog | 2026-08-30 03:10 |
...
| — | M107 Backups | 1 | V0 | 🔵 En curso — reservado | Deepseek V4 Flash | ox-alpha (Cline) | M59 🟡 (nÃºcleo OK) | Scripts PS backup/verify/restore/registro de tarea + backup.yml (guard secrets) + docs retenciÃ³n/restauraciÃ³n/desastres. AdaptaciÃ³n a rutas reales (sin disco E:, destino D:\Backups) | 2026-08-31 |

### Bloque `Reserva actual` obligatorio en cada checklist de módulo

Al reservar un mÃ³dulo, el agente debe agregar este bloque al inicio de `DOCUMENTACION/{ID}-Modulo/plan-actual/05-Checklist.md` y cambiar su estado a `🔵 En curso`:
