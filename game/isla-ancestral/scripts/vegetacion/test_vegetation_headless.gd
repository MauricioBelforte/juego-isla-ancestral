...
func _run() -> void:
	...
	# Config real (data-driven): 7 biomas = 5 anillos + humedal + cercanias (iter. 5)
	_check("Config con 7 biomas", config.size() == 7)
	var tipos_playa: Array = VM.tipos_para_bioma("playa")
	_check("Playa: 3 tipos con palmeras", tipos_playa.size() == 3 and String(tipos_playa[0]).contains("palmera"))
	_check("Bosque: 4 tipos", VM.tipos_para_bioma("bosque").size() == 4)
	_check("Densidad playa = 12 (config actual)", VM.densidad("playa") == 12)
	...
	_check("Posiciones deterministas (semilla 42)", pos1.size() == 12 and pos1[0] == pos2[0] and pos1[5] == pos2[5])
