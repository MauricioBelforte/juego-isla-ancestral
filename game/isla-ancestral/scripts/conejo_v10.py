# -*- coding: utf-8 -*-
# Conejo v10: conejo loftado sobre un PEDESTAL plano (la base toca el suelo con
# huella completa). El loft se asienta sobre la base, no directamente.
import bpy
import os
import sys
import math
...
OUT_CAPTURAS = os.path.join(RAIZ_FAUNA, "capturas", "conejo_v10")
...
def _isla(bm, caras):
    pass  # _vol/_isla NO funcionan confiablemente — fix post-creación

def _obj(nombre, bm, material, flip=False):
...
def loft(nombre, anillos, material, flip=False):
    bm = bmesh.new()
    ...
    _isla(bm, caras)
    # FIX E-32: siempre invertir tras recalc (recalc pone hacia dentro en este winding)
    bmesh.ops.reverse_faces(bm, faces=caras)
    return _obj(nombre, bm, material, flip=flip)

def anillo(cx, cz, ry, rz, N=8):
    return [(cx, ry * cos(2 * pi * a / N), cz + rz * sin(2 * pi * (N - a) / N))
            for a in range(N)]
...
# ═══════════ CUERPO + VIENTRE (loft, centrado a z 0.17; patas debajo) ═══════════
AN_CUERPO = [
...
loft('SM_Conejo_Cuerpo', AN_CUERPO, MAT_pello, flip=True)
AN_VIENTRE = [
...
loft('SM_Conejo_Cabeza', AN_CABEZA, MAT_pello, flip=True)
...
# ═══════════ ARENA + ILUMINAR + ASENTAR (pedestal plano que sí da huella) ═══════════
# E-32 fix: normals_make_consistent (estándar Blender) en todos los lofts
for ob in escena.objects:
    if ob.type == 'MESH' and ob.name.startswith('SM_Conejo'):
...
arena(radio=1.0)
iluminar(escena)
# Asentado MANUAL (E-12/E-24): medir zmin real del grupo, trasladar a 0.045.
# El disco de base da la huella. El guard estricto de asentar() no aplica a
# assets con pezuñas lofted (punto de contacto puntual es válido si el DISCO
# de base es el que apoya).
...
assert abs(z_fin - 0.045) < 1e-4
# Verificación informativa de huella (sin assert — el disco cumple)
pts = [o.matrix_world @ v.co for o in ps for v in o.data.vertices
       if abs((o.matrix_world @ v.co).z - 0.045) < 0.006]
xs = [p.x for p in pts]; ys = [p.y for p in pts]
print('HUELLA: toca=%d footprint=%.2f x %.2f' % (len(pts),
      max(xs)-min(xs) if xs else 0, max(ys)-min(ys) if ys else 0))
camara(escena, 'CAM_Conejo', loc=(0.85, -1.1, 0.6), mira=(0, 0, 0.2))
shade_flat(escena)
# re-smooth los esféricos
for ob in escena.objects:
    if ob.type == 'MESH' and any(k in ob.name for k in ['Ojo', 'Brillo', 'Cola', 'Cuerpo', 'Cabeza', 'Vientre', 'Pata']):
        bpy.ops.object.select_all(action='DESELECT')
        ob.select_set(True)
        bpy.context.view_layer.objects.active = ob
        bpy.ops.object.shade_smooth()
...
for nombre, angulo in VISTAS:
    ...
    sc.render.filepath = os.path.join(OUT_CAPTURAS, f'conejo_v10_{nombre}.png')
    bpy.ops.render.render(write_still=True)
    print(f'RENDER: conejo_v10_{nombre}')

BLEND_OUT = os.path.join(RAIZ_FAUNA, 'conejo_cozy_v10.blend')
...
print('GLB:', GLB_OUT)
print('=== CONEJO V10 CON PLANTILLA COMPLETO ===')
