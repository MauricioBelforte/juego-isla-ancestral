...
var _base_rot_patas: Dictionary = {}  # v14: reposo de patas/pies del GLB
var _altura_ajuste: float = 0.0  # v14: correccion medida al aterrizar (bounding)
...
# v13 — ejes del plegado verificados con dump_glb.py (span de las alas ±Z,
...
const YAW_PLEGADO := 1.45    # barre el span lateral hacia la cola (-X)
const ROLL_PLEGADO := 1.50   # superficie vertical contra el flanco (rot Z)
const PITCH_PLEGADO := 0.05  # v15: caida del ala aprobada (medida en demo)
const POSE_PARADA := 0.42   # v15: cuerpo ERGUIDO aprobado (cola ~4 cm del suelo)
const ALTURA_TIERRA := -0.16  # v14: se recalibra con bounding real al aterrizar
...
func _ready() -> void:
	...
	# v14 — capturar reposo de patas/pies (pose de vuelo del GLB) para el
	# replegado al despegar.
	for pata in _patas:
		_base_rot_patas[pata] = pata.rotation
	for pie in _pies:
		_base_rot_patas[pie] = pie.rotation
...
func _resolver_nodos() -> void:
	...
	# v14 — PATAS Y PIES: re-parentar cada pie a su pata para que el
	# despliegue a vertical los acompae (en vuelo van pegados al extremo
	# de la pata recogida).
	for i in 2:
		var pata := _buscar_hijo("Pata_%d" % i)
		if pata:
			_patas.append(pata)
			var pie := _buscar_hijo("Pie_%d" % i)
			if pie:
				_pies.append(pie)
...
func _iniciar_despegue_vertical() -> void:
	...
	_finalizando = false
	_plegar_patas()  # v14: patas de vuelta a la pose de vuelo (recogidas)
...
func _aterrizar() -> void:
	...
	if modelo:
		...
		modelo.rotation.z = POSE_PARADA
	# v14 — calibrar el asentado con el bounding REAL del modelo con las
	# patas desplegadas (medicion de la demo v14f, recursiva incluyendo
	# los pies hijos de las patas): el punto mas bajo de patas/pies queda
	# 5 mm sobre el piso. Cero constantes magicas de altura.
	_desplegar_patas(1.0)  # aplicacion instantanea para poder medir
	_recalibrar_altura_tierra()
	print("[Gaviota] ATERRIZADA en el suelo — caminatas: %d" % _caminatas_objetivo)
	_elegir_destino_suelo()


## v14 — Llevar el eje del cono de cada pata (local Y tras el export
## Y-up; verificado con dump_glb.py) a (0,-1,0) MUNDO con la rotacion mas
## corta (quaternion). Los pies son hijos de las patas y acompanan; se
## aplanan contra el plano del suelo con el mismo metodo. factor 1.0 =
## instantaneo (aterrizaje), < 1.0 = progresivo (lerp suave).
func _desplegar_patas(factor: float) -> void:
	for pata in _patas:
		...
		if eje_mundo.length() > 0.001 and eje_mundo.dot(objetivo) < 0.999:
			...
			pata.quaternion = pata.quaternion.slerp(objetivo_local, factor)
	for pie in _pies:
		var eje_largo: Vector3 = pie.global_basis * Vector3(1.0, 0.0, 0.0)
		var plano := Vector3(eje_largo.x, 0.0, eje_largo.z)
		if plano.length() > 0.001:
			var objetivo_p := plano.normalized()
			var eje_actual := eje_largo.normalized()
			if eje_actual.dot(objetivo_p) < 0.999:
				var q_giro := Quaternion(eje_actual, objetivo_p)
				...
				pie.quaternion = pie.quaternion.slerp(objetivo_local, factor)


## v14 — Medir el punto mas bajo de patas/pies (en mundo, AABB esquinas
## transformadas, recorrido RECURSIVO del modelo porque los pies son
## hijos de las patas) y ajustar modelo.position.y para que quede a 5 mm.
func _recalibrar_altura_tierra() -> void:
	var modelo := get_node_or_null("Modelo") as Node3D
	if modelo == null:
		return
	var min_patas := INF
	var cola: Array[Node3D] = [modelo]
	while not cola.is_empty():
		var nodo: Node3D = cola.pop_back()
		for h in nodo.get_children():
			if h is Node3D:
				cola.append(h)
		if nodo is MeshInstance3D and nodo != modelo:
			if nodo.name.to_lower().contains("pata") or nodo.name.to_lower().contains("pie"):
	...
	if is_finite(min_patas):
		var delta_y: float = (global_position.y + 0.005) - min_patas
		modelo.position.y += delta_y
		print("[Gaviota] patas asentadas: min_y %.3f → ajuste %+.3f" % [min_patas, delta_y])


## v14 — Plegar las patas de vuelta a la pose de VUELO del GLB (reposo):
## restaurar la rotacion base capturada en _ready (antes de cualquier
## despliegue). Llamada al despegar.
func _plegar_patas() -> void:
	for pata in _patas:
		if _base_rot_patas.has(pata):
			pata.rotation = _base_rot_patas[pata]
	for pie in _pies:
		if _base_rot_patas.has(pie):
			pie.rotation = _base_rot_patas[pie]
...
func _paso_caminando(delta: float) -> void:
	...
	if _salto_pendiente:
		...
		_v_salto -= 14.0 * delta
		velocity.y = _v_salto
		# v16: empuje frontal durante el hop — el apex mas alto por si solo
		# no cruza el escalon: la gaviota caia en el borde ANTES de pasar.
		# x2.5 de avance mientras dura el arco para superar el labio del
		# escalon (mismo dir/velocidad base, sin tocar la caminata normal).
		velocity = dir * velocidad_caminata * 2.5
		velocity.y = _v_salto
	else:
	...
	if is_on_wall():
		...
		if _t_muro > 0.3 and not _salto_pendiente:
			...
			_v_salto = 6.5  # v16: apex ~1.51 m — cruza el escalon de 1 m con margen (5.4 llegaba al borde y caia)
			_hops += 1
			if _hops >= 3:
				_hops = 0
				_reintentos_dir += 1
...
func _animar(delta: float) -> void:
	match _estado:
		...
		ESTADO_CAMINANDO, ESTADO_PAUSA_SUELO:
			_bater_alas(delta, 0.0, 0.0, true)        # plegadas hacia ATRAS (v11)
			_desplegar_patas(clampf(3.0 * delta, 0.0, 1.0))  # v14: patas a vertical (suave)
			var modelo := get_node_or_null("Modelo")
			if modelo:
				var activo: bool = _estado == ESTADO_CAMINANDO
				# v13: head-bob con pitch en rot Z (eje del cuerpo en el GLB);
				# rotation.x era roll lateral y la volcaba de costado.
				var paso: float = absf(sin(_t * 6.0))
				modelo.rotation.z = POSE_PARADA - (0.05 * paso if activo else 0.0)
				modelo.position.y = ALTURA_TIERRA + _altura_ajuste + (0.02 * paso if activo else 0.0)
				modelo.position.x = -0.02 * paso * (1 if activo else 0.0)
...
func _bater_alas(_delta: float, frecuencia: float, amplitud: float, plegar: bool) -> void:
	if _alas.size() == 2:
		for i in 2:
			...
			if plegar:
				# v13 — PLEGADO DE AVE (ejes verificados con dump_glb.py):
				# el GLB exportado queda con el span de las alas en ±Z y el
				# pico en +X (Godot compose R = Ry·Rx·Rz, Z se aplica
				# primero). Dos rotaciones:
				#   ROLL (rotation.z, eje del span, MISMO signo en ambas):
				#     verticaliza la superficie del ala contra el flanco.
				#     Con signo negado por lado una ala quedaba plegada y la
				#     otra LEVANTADA (bug v13a de la demo).
				#   YAW (rotation.y, barrido, signo negado por lado): lleva
				#     el span ±Z hacia -X (la cola). Ala_L (span +Z) yaw -,
				#     Ala_R (span -Z) yaw +.
				# v15 — PITCH de caida (aprobado por el usuario en demo v15c
				# con cuerpo erguido 0.42): puntas 20 cm POR DEBAJO del
				# hombro, pegadas al flanco. Signo negado por lado (rot X
				# manda Z→−Y con angulo +: sube el span −Z, baja el +Z).
				var objetivo_roll: float = base.z - ROLL_PLEGADO
				var objetivo_yaw: float = base.y + YAW_PLEGADO * lado
				var objetivo_pitch: float = base.x + PITCH_PLEGADO * lado
				ala.rotation.z = lerp(ala.rotation.z, objetivo_roll, 3.0 * _delta)
				ala.rotation.y = lerp(ala.rotation.y, objetivo_yaw, 3.0 * _delta)
				ala.rotation.x = lerp(ala.rotation.x, objetivo_pitch, 3.0 * _delta)
			else:
				# VUELO: aleteo en rotation.x (barrido arriba/abajo del
				# span ±Z — pitch alrededor del eje de la cuerda). Se
				# aplican ANTES del yaw/roll residuales: aca base.x + seno.
				ala.rotation.z = base.z
				ala.rotation.y = base.y
