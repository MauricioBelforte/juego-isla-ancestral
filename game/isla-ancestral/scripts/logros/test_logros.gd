...
# % real, logros ocultos, persistencia).
...
func _test_persistencia() -> void:
	...
	_check((data.desbloqueados as Array).size() >= 2, "logros persistidos")
	# Round-trip con logro de catálogo viejo
	var con_viejo: Dictionary = data.duplicate(true)
	(con_viejo.desbloqueados as Array).append("logro_viejo")
	_ach.restore_save_data({})
