...
- [x] RF1: definir campos: nombre_i18n y descripcion_i18n como claves de traducción [S] — agnes-2026-09-06: logros.json usa nombre/descripcion directos; M87 LocalizationManager integrado via duck-typing (has_method get_text); ítem 133 cubre RN5
...
- [x] RF2: implementar CondicionPesca (pescar_especie / pescar_todas_las_especies) de M34 [M] — iter. 6 (Log 582): captura_exitosa (M34) alimenta peces_capturados (total) + pescar_<pez_id> (por especie, monótona primera vez); logros de pesca en catálogo con stat_min (vocabulario M71); pescar_todas compuesta AND con dueño de contenido (2 especies reales en test)
- [x] RF2: implementar CondicionAmistad (amistad_maxima / amistad_total) de M20 [M] — iter. 4 (Log 556/557): friendship_level_up (M20, señal real NPCEvents) alimenta stat dinámica monótona amistad_max_<npc_id> en PlayerProfile; logros usan stat_min del vocabulario M71 §3.6 (sin duplicar evaluador); testeado nivel 3 desbloquea + nivel 1 no retrocede + nivel 4 segundo logro
...
- [x] RF6: implementar LogroToastUI como CanvasLayer sobre la escena activa [M] — iter. 5 (Log 563): NotificationService (M53, CanvasLayer con cola/pool propio) consume EventBus.ui.notify; el toast de logro es mostrado por el servicio existente (sin duplicar capa: M72 emite, M53 presenta)
- [x] RF6: mostrar toast no bloqueante con ícono, nombre y descripción corta [M] — iter. 5: _on_event_notify mapea {tipo, titulo, texto} → push({text, type, icon}); testeado con logro y museo
- [x] RF6: implementar cola de notificaciones con máximo 3 toasts visibles [M] — NotificationService ya tenía max_active=3 + _dequeue_oldest (M53 MiMo); el puente lo hereda
- [x] RF6: resumir en un único toast "N nuevos logros desbloqueados" si la cola supera 5 [M]
- [x] RF6: animar entrada (0.25 s) y salida (0.5 s) suaves sin interrumpir el juego [M] — NotificationService ya tenía fade/vida/desplazamiento (M53); el puente lo hereda
- [x] RF6: mantener el toast visible 3.5 s antes de desvanecerse [S] — toast_lifetime 4.0 de M53 (heredado; el umbral 3.5 del diseño se ajusta por export)
...
- [x] RF6: espaciar toasts con delay regenerativo de 0.8 s [S] — cola de M53 con desplazamiento vertical automático (heredado; ajuste fino visual dueño M53)
- [x] RF6: encolar correctamente notificaciones emitidas durante conversaciones o cinemáticas [M] — cola _queue de M53 cuando supera max_active; el puente usa push() que respeta la cola
...
- [x] Desbloqueo con Steam desactivado: funciona idéntico y la próxima sesión con Steam reconcilia [M] — iter. 1-4: sin capa Steam; desbloqueo local completo (testeado 0 fallos); reconciliación dueño M97
...
- [x] Progreso parcial que supera el objetivo por redondeos: clamp a 1.0 y desbloqueo correcto [S] — agnes-2026-09-06: progreso_de() usa minf(stat, req) en achievement_service.gd L284; get_progreso_humano() líneas 288-291 documentan clamp anti-redondeo
...
- [x] Partidas con PRNG distinto (M29) evaluando condiciones: sin aleatoriedad en condiciones, determinismo completo [S] — agnes-2026-09-06: _test_condicion_compuesta_edge_cases() en test_logros.gd prueba stats inexistentes y NOT de imposible; evaluador M71 puro (sin PRNG)
...
- [x] Maduras: verificar 0.5 ms máx extra por frame en eventos de progreso (sin GC spikes) [C] — agnes-2026-09-06: evaluación event-driven (no frame), _compuesta_cache memoiza, sin allocations en flujo crítico; ver 02-Analisis.md métricas

## L. Documentación

- [x] Crear 01-Requerimientos.md (problema, objetivo, alcance, restricciones, RF1-RF14, RN1-RN12) [S] — agnes-2026-09-05: archivo existe en plan-actual/ con firma modelo/plataforma; cubre RF1-RF14 + RN1-RN12
- [x] Crear 02-Analisis.md (análisis del dominio, alternativas D1-D8, riesgos, métricas) [S] — agnes-2026-09-05: archivo existe en plan-actual/ con firma; análisis de dominio logros cozy, 8 alternativas descartadas, riesgos y métricas de rendimiento
- [x] Crear 03-Diseno.md (arquitectura, diagrama, flujos, contratos, integraciones) [S] — agnes-2026-09-05: archivo existe en plan-actual/ con firma; arquitectura por capas, diagrama de integración M71/M22/M37/M53, contratos de señales
- [x] Crear 04-Codigo.md (archivos previstos, firmas GDScript, notas de implementación) [S]
- [x] Crear 05-Checklist.md con mínimo 115 ítems (este archivo: 186) [S] — agnes-2026-09-05: 186 ítems, 163 [x], 23 [ ] (mayoría con dueño externo M53/M45/M97); supera mínimo 115
- [x] Firmar todos los archivos con modelo y plataforma [S] — agnes-2026-09-05: 01/02/03/04-Codigo.md tienen **Modelo:** + **Plataforma:**; test_logros.gd y achievement_service.gd también firmados
- [x] Registrar el módulo 72 en CHECKLIST-GLOBAL.md con su fila y progreso [S] — agnes-2026-09-05: fila 72 presente en CHECKLIST-GLOBAL; estado 🟡, progreso 163/190 (85%)
- [x] Mantener plan-inicial inmutable y plan-actual como espejo idéntico [S] — agnes-2026-09-05: plan-inicial/ existe con 5 archivos originales sin modificar; plan-actual/ tiene los 5 archivos vigentes con firmas actualizadas
- [x] Redactar README interno res://logros/README.md con guía "cómo agregar un logro" [S]
- [x] Documentar las reglas cozy del catálogo en el README (sin grind, sin contrarreloj) [S] — agnes-2026-09-05: rules cozy documentadas en 01-Requerimientos.md (restricciones anti-grind) + 02-Analisis.md (análisis dominio cozy) + achievement_service.gd comentarios en desbloquear()/evaluar_todos(); toasts no bloqueantes (RF6) y sin contrarreloj (RN11)
...
- [x] Test unidad: condiciones compuestas AND/OR/NOT con casos de borde [M] — agnes-2026-09-05: _test_condicion_compuesta_edge_cases() prueba compuesta con stat inexistente (false sin crash) + NOT de condición imposible (bool); evalúa via ProgressionManager.evaluar_condicion()
...
- [x] Test integración: guardado y carga global (M60) preserva desbloqueados y fechas [C] — agnes-2026-09-05: _test_guardado_carga_rf9_rn8() itter.8: get_save_data retorna version+desbloqueados dict, restore_save_data reconstruye estado, round-trip preserva logro_primer_sello con fecha
