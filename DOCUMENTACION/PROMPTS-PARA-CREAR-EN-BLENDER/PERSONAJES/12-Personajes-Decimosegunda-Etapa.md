Seguimos con **A: corregir la revisión artística de Ceniza, Coral y Aurora**.

En lugar de acumular parches sobre los detalles defectuosos, esta versión **reemplaza la función de decoración** y vuelve a partir de los ALTA originales.

### Correcciones aplicadas

- Los accesorios de manos utilizan **coordenadas relativas a la mano correspondiente**.
- Los adornos izquierdos y derechos se incorporan a su brazo correcto.
- No vuelve a crear mochilas, linternas, faroles, telescopios o frascos que ya existen en el generador base.
- La postura encorvada se aplica a **Obsidiana, Alga, Hielo y Nieve**; no a Vulcania.
- Los informes enumeran **solo los detalles realmente añadidos**.
- Guarda el resultado en **`REVISION_03`**, sin sobrescribir los originales ni `REVISION_02`.

**Alcance:** corrige esta pasada de accesorios. No repara automáticamente geometría errónea que ya esté dentro del ALTA original.

---

# 1. Archivo `revision_artistica_restantes_v3.py`

Guárdalo junto al anterior:

```text
GENERADOR_NPC/
├── revision_artistica_restantes.py
└── revision_artistica_restantes_v3.py
```

Reutiliza del primer archivo las primitivas, la carga de escenas y el renderizado. **No ejecuta su decoración ni su lote anterior.**

Ejecuta:

```bash
blender --background --factory-startup --python revision_artistica_restantes_v3.py
```

> Utiliza una instancia dedicada: el script limpia la sesión entre personajes.

```python
# ================================================================
# REVISION ARTISTICA 03 — CENIZA / CORAL / AURORA
# Blender 4.2+
#
# Requiere:
#   revision_artistica_restantes.py
#
# Reutiliza sus utilidades, NO su decoracion ni su lote.
#
# Entrada: ALTA original.
# Salida: carpeta del NPC / REVISION_03/
#
# Sin materiales nuevos, sin meshes finales adicionales.
# No reemplaza herramientas ya existentes.
# ================================================================
...
SCRIPT_ANTERIOR = (
    CARPETA_SCRIPT / "revision_artistica_restantes.py"
)

BASE_EXPORT = Path.home() / "NPC_EXPORT"
...
├── SM_NPC_BRASA.blend
├── SM_NPC_BRASA.glb
├── REVISION_02/              ← permanece intacta
└── REVISION_03/
    ├── SM_NPC_BRASA.blend
    ├── SM_NPC_BRASA.glb
    ├── REVISION_03.json
    └── cap_NPC_BRASA_v3_...png
```

La reejecución parte siempre del original: **no acumula accesorios entre ejecuciones**. Si falla un personaje, consulta su archivo `ERROR_REVISION_03.json`; cualquier salida anterior de esa carpeta debe considerarse antigua.

---

# 3. Comparar las versiones en las láminas de contacto

Para utilizar el script de contacto con versiones diferentes por isla, añade esta configuración:

```python
REVISION_POR_ISLA = {
    "01_RAIZ": "REVISION_02",
    "02_CENIZA": "REVISION_03",
    "03_CORAL": "REVISION_03",
    "04_AURORA": "REVISION_03",
}
```

Y sustituye su función `ruta_npc()` por:

```python
def ruta_npc(carpeta_isla, identificador, nombre):
    carpeta = BASE / carpeta_isla / f"{identificador}_{nombre}"
    revision = REVISION_POR_ISLA.get(carpeta_isla)

    if revision:
        carpeta = carpeta / revision

    return carpeta / f"SM_NPC_{nombre}.blend"
```

En el registro, cambia también la descripción de fuente:

```python
"fuente": "REVISION_POR_ISLA",
```

Esto permite comparar **Raíz v2 y las otras tres islas v3** sin renombrar carpetas ni confundir revisiones.

## Qué queda después de este parche

**A queda cubierto como revisión de código**, pendiente de ejecutar y revisar sus imágenes. No lo doy por validado visualmente.

El siguiente bloque de la recomendación es **B: `NPC_BASE` en Godot**, con:

- `CharacterBody3D`.
- Colisión única, independiente del LOD.
- `NavigationAgent3D`.
- Estados **Idle → Caminar → Hablar**.
- Interacción del jugador.
- Animación CAMINAR ligada a la velocidad real.
- Un único controlador reutilizable para los 35 NPCs.