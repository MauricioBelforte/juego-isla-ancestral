# Primera entrega: Luna — Isla Raíz, NPC 01/35
Empiezo por Luna, la pintora, como primer personaje de la serie. El diseño incluye moño con cinta rosa, falda plisada crema, delantal con manchas, pincel y paleta. Es humana: “Gato” se interpreta como referencia de personalidad, no como anatomía animal.

Te entrego un script autónomo que crea el personaje y, al ejecutarlo, guarda su .blend, exporta el .glb, renderiza las seis vistas y escribe el informe de geometría. Esta entrega implementa Luna; no presenta los otros 34 como terminados.

Decisiones técnicas importantes
Hay tres incompatibilidades en la especificación que conviene resolver explícitamente:

8 meshes + Empty = 9 objetos de Blender. Mantengo los 8 objetos de malla de tu jerarquía y un Empty adicional. Si el límite cuenta también el Empty, hay que reducir a siete meshes.
Los pivotes proporcionados pertenecen a la receta anterior, de aproximadamente 1,55 m. El script los utiliza como referencias y los escala junto con la geometría para alcanzar 1,8 m. Conservar sus coordenadas literales después de aumentar la anatomía desplazaría las articulaciones.
Pivotes articulares y “All Transforms” no son equivalentes. La geometría lleva su tamaño definitivo horneado; todos los objetos tienen rotación cero y escala uno, pero sus posiciones conservan los pivotes. Aplicar también la posición trasladaría los orígenes.
Los accesorios se incorporan a los ocho meshes existentes. Para no superar seis materiales, sus colores —incluidas las manchas de pintura— se almacenan como colores por esquina de cara, sin nodos procedurales que necesiten horneado.

Cómo ejecutarlo
Recomendado: Blender 4.2 LTS o posterior, en una instancia dedicada.

Guarda el código como crear_luna.py.
Cambia OUTPUT_DIR.
Ejecuta:
Bash

blender --background --factory-startup --python crear_luna.py
También puedes usar Scripting → Run Script, pero guarda primero cualquier trabajo abierto: el script limpia los objetos de la sesión para generar un .blend individual.

El resultado es una base procedural detallada para aprobación visual, no una malla ya revisada manualmente ni un personaje riggeado. El script informa las fronteras abiertas y no confunde piezas superpuestas con una superficie soldada.
...
# Archivos que genera al ejecutarse
text
...
└── 01_RAIZ/
    └── 01_LUNA/
        ...
        ├── SM_NPC_LUNA.glb
        ├── SM_NPC_LUNA_RESUMEN.json
        ├── cap_NPC_LUNA_v1_<fecha>_0.png
        ├── cap_NPC_LUNA_v1_<fecha>_1.png
        ├── cap_NPC_LUNA_v1_<fecha>_2.png
        ├── cap_NPC_LUNA_v1_<fecha>_3.png
        ├── cap_NPC_LUNA_v1_<fecha>_4.png
        └── cap_NPC_LUNA_v1_<fecha>_5.png
La reejecución reemplaza el .blend, el .glb y el resumen; no duplica objetos. Las capturas anteriores permanecen como historial, porque llevan fecha y hora.

Qué está implementado y qué requiere revisión
Aspecto	Estado
Luna humana, moño, cinta, falda plisada y delantal	Implementado
Pincel y paleta en las manos	Implementado como geometría estática
Altura de 1,8 m con escala de objetos uno	Comprobación automática
8 meshes, Empty adicional, 6 materiales	Comprobación automática
Máximo de 6.000 tris	El script detiene la exportación si se excede
Seis vistas separadas 60°	Automatizado
Colores de piel SK-02	Exactos según tu tabla
Cabello HR-02 y ojos EY-02	Tonos propuestos; faltan sus hex oficiales
Topología continua para deformación	Pendiente: son piezas superpuestas
Ausencia de intersecciones visibles	Revisión de las seis capturas
Importación comprobada en Godot 4.7.2	Pendiente, no ejecutada aquí
En Godot
Mantén la instancia a escala (1, 1, 1): la altura ya está horneada en metros. No hay que volver a multiplicarla por 1,8.
El frente +Y de Blender se convierte en -Z con la conversión glTF utilizada.
Comprueba que los materiales importados usan Vertex Color → Use as Albedo. Los colores de ropa y accesorios dependen de COLOR_0.
Para modificar un material concreto, usa un override por superficie, no material_override sobre el mesh completo: este último sustituiría también piel, herramienta u otros materiales presentes.
Las herramientas están integradas en las manos para respetar el presupuesto estático. Para equiparlas o soltarlas durante el juego habrá que separarlas del NPC o tratarlas mediante el rig.
Orden restante de producción: Rocky → Coral → Chef → Fin → Flora → Sage → Merc → Nana → Carp → Melodía → Roca; después Ceniza, Coral y Aurora. No conviene propagar a los 35 una base sin revisar primero sus proporciones, lectura facial y exportación. 