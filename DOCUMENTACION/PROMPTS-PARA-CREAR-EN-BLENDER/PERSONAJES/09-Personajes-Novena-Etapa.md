Entendido. **Seguimos construyendo la guía completa y dejamos los retoques visuales para el final**, pero mejorando la base técnica para que esos retoques no obliguen a rehacer todo.

El siguiente paso es un **rig deformable con pesos automáticos controlados**, más tres animaciones:

- **IDLE:** respiración y balanceo leve.
- **CAMINAR:** ciclo en el sitio, sin desplazamiento del nodo raíz.
- **SALUDO:** gesto con el brazo izquierdo; configurable según las herramientas.

Este script **sustituye al rig rígido anterior**. No necesitas ejecutar ambos.

# 1. Qué cambia

| Parte | Tratamiento |
|---|---|
| Torso | Mezcla entre pelvis y pecho |
| Cabeza, pelo y ojos | Siguen al hueso de cabeza, cuyo origen está en el cuello |
| Brazos | Pesos progresivos entre brazo, antebrazo y mano |
| Piernas | Pesos progresivos entre muslo, espinilla y pie |
| Herramientas integradas en brazos | Sus islas de geometría se vinculan rígidamente a la mano |
| Faldas y delantales | Permanecen principalmente vinculados a pelvis/torso; retoque posterior |
| Accesorios de cabeza | Conservan su relación con el cráneo |

**Importante:** las herramientas deben seguir a la mano, no deformarse con el codo. El script distingue la isla principal del brazo de las piezas adicionales para aproximar ese comportamiento.

La exportación animada se guarda por separado en `RIG_DEFORMABLE`. Los GLB estáticos originales conservan `Animation: OFF`.

---

# 2. Script `rig_deformable_npcs.py`

Autónomo, para **Blender 4.2 LTS o posterior**. Utiliza inserción de claves mediante huesos, evitando construir directamente las F-curves heredadas.

Guárdalo y ejecuta:

```bash
blender --background --factory-startup --python rig_deformable_npcs.py
```

Por defecto procesa los ALTA originales. Si prefieres las revisiones, cambia `USAR_REVISION_02`.

> El script limpia la sesión entre personajes. Ejecútalo en una instancia dedicada. No lo he ejecutado aquí: los archivos y estadísticas se obtienen al correrlo en Blender.

```python
# ================================================================
# NPC COZY — RIG DEFORMABLE + IDLE / CAMINAR / SALUDO
# Blender 4.2+
#
# Entrada: ALTA original o REVISION_02 seleccionada expresamente.
# Salida: carpeta del NPC / RIG_DEFORMABLE/
#
# No sobrescribe originales.
# No utiliza automatic weights / bone heat.
# Conserva COLOR_0 y materiales.
# ================================================================
...
# 3. Cómo revisar las animaciones en Blender

El `.blend` se guarda en pose neutra y con las pistas NLA silenciadas para evitar que se mezclen entre sí.

Para reproducir un clip:

1. Selecciona el Armature.
2. Abre **Nonlinear Animation**.
3. Activa **solo una pista**:
   - `IDLE`
   - `CAMINAR`
   - `SALUDO`
4. Reproduce la línea de tiempo.

No actives simultáneamente las tres pistas para evaluar el movimiento.

### Duración

| Clip | Fotogramas | Duración aproximada |
|---|---:|---:|
| IDLE | 1–61 | 2 segundos |
| CAMINAR | 1–31 | 1 segundo |
| SALUDO | 1–61 | 2 segundos |

IDLE y CAMINAR tienen poses inicial y final coincidentes. En Godot deben configurarse como **Loop Linear**; SALUDO no debe repetir.

---

# 4. Integración con lo que ya tenemos

En `NPCAnimado`:

- `high_scene`: nuevo `SM_NPC_<NOMBRE>_RIG.glb`.
- `source_idle`: nombre real del clip IDLE importado.
- `source_greeting`: nombre real de SALUDO, si existe.

El controlador anterior ya permite probar IDLE y SALUDO. **CAMINAR queda exportado**, pero su reproducción debe conectarse al movimiento real del NPC en el siguiente paso.

Para caminar:

- El nodo de lógica mueve al personaje.
- La animación mueve las extremidades.
- No se desplaza `ROOT` dentro del clip.

Así evitamos sumar dos desplazamientos y mantenemos el control de navegación en Godot.

---

## Qué queda preparado y qué retocaremos al final

**Preparado:**

- Esqueleto común de 16 huesos.
- Skinning con pesos normalizados.
- Máximo de dos influencias por vértice en esta receta.
- Codos y rodillas deformables.
- Clips separados.
- Geometría y colores conservados.
- Exportación animada independiente de los estáticos.

**Retoques posteriores:**

- **Faldas y túnicas:** el ciclo puede atravesarlas porque aún no tienen huesos propios.
- **Pies:** no hay IK ni bloqueo de contacto; puede existir deslizamiento o penetración leve.
- **Herramientas:** la clasificación por islas es una aproximación; algunos adornos o puños pueden requerir reasignación.
- **SALUDO:** debe adaptarse cuando la mano izquierda sostiene una paleta, libro o bolsa.
- **Topología:** el rig no añade loops; una rodilla con poca geometría seguirá doblando de forma angulosa.
- **LOD:** MEDIA y BAJA anteriores continúan siendo estáticas. No adquieren pesos automáticamente.

Con esto avanzamos de un prototipo de piezas rígidas a una **base animable común para el catálogo**, manteniendo separados los originales, las revisiones artísticas y los archivos preparados para animación.