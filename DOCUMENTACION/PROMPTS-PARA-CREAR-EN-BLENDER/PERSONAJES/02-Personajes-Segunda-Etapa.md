Alcance de esta entrega: genera los 35 NPCs —incluyendo Luna— con anatomía común, paletas individuales, peinados, prendas principales, sombreros y herramientas diferenciadas. Son versiones procedurales para aprobación, no 35 modelos terminados y revisados manualmente. Los detalles más pequeños —bordados específicos, anzuelos, monedas, llaves, anillos y otros accesorios diminutos— quedan pendientes de una segunda pasada.

No puedo ejecutar Blender ni adjuntar aquí los resultados. El generador crea los .blend, .glb, capturas y resúmenes cuando lo ejecutes.

1. Preparación
Guarda el script completo de la respuesta anterior como:

text

crear_luna.py
Guarda el siguiente junto a él como:

text

crear_catalogo_npcs.py
Estructura:
...
├── crear_luna.py
└── crear_catalogo_npcs.py
Este segundo script necesita la versión exacta de crear_luna.py de la respuesta anterior, porque reutiliza sus primitivas y su exportador.

Ejecuta preferentemente:

Bash

blender --background --factory-startup --python crear_catalogo_npcs.py
Importante: el proceso limpia la escena entre personajes. No lo ejecutes sobre una sesión con trabajo sin guardar. Con capturas activadas se renderizan 210 imágenes, por lo que recomiendo una primera ejecución sin capturas para comprobar los 35 modelos.

2. Generador del catálogo completo
Python

# ================================================================
# CATALOGO NPC COZY — 35 PERSONAJES
# Requiere crear_luna.py de la entrega anterior.
# Blender 4.2+.
#
# Primera pasada procedural:
# anatomia + paleta + prendas principales + pelo/sombrero + herramienta.
#
# No implementa todos los microaccesorios del documento.
# No incluye rig, animaciones ni LOD MEDIA/BAJA.
#
# ATENCION: limpia la escena entre personajes.
# ================================================================
...
3. Qué diferencia a cada grupo
El generador no se limita a cambiar el color de Luna:

Grupo	Variaciones geométricas incluidas
Pintora	Conserva el modelo de Luna: falda plisada, delantal manchado, moño, pincel y paleta
Herrero / carpinteros	Delantal o chaleco, hacha o martillo, madera en la otra mano para Nácar
Exploradores	Mangas cortas, mapa enrollado y brújula; antorcha y mapa para Ceniza
Cocineros	Cofia, delantal, pañuelo y cuchara grande
Pescadores	Sombreros, caña con sedal o red pequeña
Jardinera	Trenza, sombrero de paja con cinta, guantes y rastrillo
Bibliotecaria / historiadora	Gafas, falda, chaleco y libro o pergamino
Sanadores	Cofia con cruz verde, delantal, frasco y pequeños recipientes de cinturón
Mercaderes	Bolsas, variantes de túnica, turbante y bombachos
Mineros	Casco frontal iluminado, protección, pico y lámpara de cinturón
Guardianes	Anatomía más ancha, casco, chaleco y espada o lanza roma
Músicos	Cencerro, flauta o caracola; peinados y sombreros distintos
Astrónomos / sabio	Túnica, capa, telescopio o báculo; sombrero puntiagudo para Hielo
Mensajero	Capucha abierta por delante, capa larga y farol azul
Brasa conserva la cabeza descubierta: su casco aparece en la cadera, resolviendo la indicación simultánea de “sin sombrero” y “casco de minero” de su ficha.

4. Salida organizada
text

NPC_EXPORT/
├── CATALOGO_RESUMEN.json
├── 01_RAIZ/
│   ├── 01_LUNA/
│   ├── 02_ROCKY/
│   ├── ...
...
SM_NPC_<NOMBRE>.blend
SM_NPC_<NOMBRE>.glb
SM_NPC_<NOMBRE>_RESUMEN.json
FICHA_NPC.json
cap_NPC_<NOMBRE>_v1_<fecha>_0.png
...
cap_NPC_<NOMBRE>_v1_<fecha>_5.png
5. Validación y límites de esta pasada
El presupuesto se comprueba, no se supone: si un NPC supera 6.000 tris, el exportador registra el error y no lo da por válido.
Se mantienen 8 meshes + 1 Empty, no ocho objetos totales contando la raíz.
Se utilizan seis materiales únicos, compartidos por prendas y accesorios mediante colores de vértice.
Los humanos miden 1,8 / 1,7 / 1,9 m según su ficha. En los nuevos personajes, sombreros y herramientas pueden superar esa altura.
El ancho se diferencia por arquetipo, pero no se fuerza una caja envolvente exacta de 0,6 / 0,55 / 0,7 m: esa medida varía con brazos, prendas y accesorios.
El brillo de pociones, lámparas y cristales es una sugerencia emisiva tenue. No son recipientes de vidrio ni luces que iluminen la escena.
Ropa, anatomía y herramientas contienen piezas superpuestas. No se ha realizado una unión topológica continua ni una revisión manual de intersecciones.
Los colores oficiales de cabello y ojos, bordados, bolsillos específicos y accesorios pequeños necesitan la segunda pasada.
Primera comprobación recomendada: genera primero {2, 7, 12, 24, 31, 35}. Cubren delantal, anciano, guardia, turbante, sombrero alto y capucha. Si las proporciones y la importación se ven correctas, cambia SOLO_IDS = None y ejecuta el catálogo completo.
