...
## PARTE 5 — Horizonte: factores de rendimiento y hide/toggle de tiles

### Factores que afectan el rendimiento del impostor

| Factor | Impacto | Mitigación |
|---|---|---|
| **PASO del muestreo** | Más celda = más tris, pero menos noise | PASO=64 (4× menos geom que 16m) |
| **TAMANO_TILE** | Tiles grandes = muchos tris por mesh, poca carga de streaming | 640m por tile |
| **Paredes en acantilados** | Overdraw masivo si son excesivas | Solo si vecino ≥2m más bajo |
| **Exageración de altura** | Montañas falsas tapan arena/disco | Solo h>20 |
| **FACTOR_ALTURA** | 0.85 evita z-fighting con chunks reales | No subir de 0.9 |
| **Cantidad de tiles** | Más tiles = más draw calls | Cerrar tiles lejanos |

### Hide/toggle de tiles por distancia

```gdscript
func _actualizar_visibilidad_tiles(player_pos: Vector3) -> void:
    for tile in _tiles:
        if tile == null:
            continue
        var tile_center := tile.global_position + Vector3(TAMANO_TILE / 2.0, 0, TAMANO_TILE / 2.0)
        var dist := player_pos.distance_to(tile_center)
        tile.visible = dist < TILE_OCULTAR_UMBRAL
```

### Estrategia de ocultamiento

- **< 400m**: chunks reales cubren — tiles ocultados
- **400m - 1024m**: tiles visibles (impostor cubre lo que los chunks aún no cargaron)
- **> 1024m**: tiles visibles (impostor es el único representante del terreno)

### Optimización de draw calls

Cada tile es un `MeshInstance3D` independiente → un draw call por tile. Para reducir:
- Usar **MultiMesh** si los tiles son idénticos (no lo son, tienen alturas distintas).
- Agrupar tiles en meshes más grandes (pero pierdes ocultamiento independiente).
- Usar **occlusion culling** de Godot para tiles detrás de montañas.
