...
- Salida: iter 8 cierra 3 [?] (RF7 clima, F.11 condición clima, H.16 validación de claves en runtime). +7 [ ] → 78/139 + 5 [?]. Archivos: `scripts/dialogos/dialog_graph_validator.gd` (allowlist canonica), `dialogue_manager.gd` (gate [VAL-DGV] con allowlist), `validate_all_dialogues.gd` (CLAVES_MUNDO desde validador), `test_clima_dialogo_m21.gd` (nuevo, 0 fallos).
...
> ver GUIA-GODOT/09-godot4-migracion.md §9.46 y Notas del Agente en 04-Codigo.md). Los ítems `[ ]` de este checklist
> aún no fueron relevados contra el código real por completo; los marcados `[ ]` son los
...
- [ ] M20 -> M21: DialogueManager consume EventBus.npc.gift_given por clase exacta (GiftEvaluator.Clase) y emite gift_reaction(npc, reaccion_id, clase, item) via REACCION_REGALO (R_AMADO/R_GUSTA/R_NEUTRAL/R_DUPLICADO); guarda ultima reaccion por NPC para contexto de dialogo [S]
...
- [ ] Test de salto rapido: tipeo, linea, nodo y salto completo (test_skip_m21.gd: skip hasta FIN, se detiene en OPCIONES, efectos aplicados, choose_option tras skip) 0 fallos (Log 309) [S]
- [ ] Test de ramas: cada condicion verdadera y falsa evaluada [M]
- [ ] Test de validacion: 5 grafos invalidos propositados detectados en editor [M] — iter 9 (Hy3/WorkBuddy): test_validacion_5_invalidos_m21.gd (huérfano/operador/clave/next/goto) 0 fallos
...
- [ ] Test de reinicio del diálogo tras completarlo (test_dialogos.gd: _test_reinicio_dialogo — 0 fallos) [S]
