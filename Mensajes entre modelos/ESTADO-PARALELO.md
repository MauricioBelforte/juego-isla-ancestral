...
| Implementación base del motor | GitHub Copilot | VS Code | ✅ COMPLETADO — M04 Game Engine | ✅ Validación Godot 4.7.2 finalizada (2026-08-26 01:30). Motor arrancable, escenas cargan sin errores, VoxelTerrain + colisión operativos. M05/M07 habilitados. |
...
| 124 — Contenido Generado por Usuarios | Deepseek V4 Flash | 2026-08-21 | ✅ Documentado (106/106): GATE post-V2, tipos (fotos M56 2K + blueprints M18/M17), moderación hash+IA+humana con SLA 24 h, almacenamiento CDN con límites por usuario, reportes por categoría, privacidad/derecho al olvido (M80), copyright del usuario + licencia al servicio, cláusula ToS (M125) y backups RPO 24 h. DELEGABLE. Pendiente QA cruzado |
| 144 — Después del Lanzamiento | Deepseek V4 Flash | 2026-08-21 | ✅ Documentado (105/105): tablero semanal de 9 métricas (reviews, bugs, perf, economía, dificultad, tutorial, onboarding, retención, contenido), hotfix <72 h + patch mensual con changelog, mejoras por evidencia, contenido ignorado <10% → mejorar/archivar, GATE de contenido (M120), backups RPO 24 h, soporte SLA 72 h, documentación viva y reporte mensual a M136. DELEGABLE. Pendiente QA cruzado |
| 98 — Trailer | Deepseek V4 Flash | 2026-08-21 | ✅ Documentado (101/101): 3 tráilers (teaser anuncio, gameplay Steam, lanzamiento M143), guion y storyboard 8-10 planos, música M41 licenciada por track, 100% gameplay real sin pre-render, los 6 valores (mundo, construcción, NPC, puzzles, viajes, misterio), anti-spoilers (sin sellos/templo final/epílogo), subtítulos 6 idiomas, versiones 16:9 + 9:16, thumbnails y derechos auditados (M84). DELEGABLE. Pendiente QA cruzado |
| 79 — Legal Contratos | Deepseek V4 Flash | 2026-08-21 | ✅ Documentado (103/103): plantilla única de contrato de contribución + 8 anexos por rol, cesión de PI al estudio con autoría conservada, remuneración fija + royalties opcionales (música/escritura), confidencialidad + NDA, terminación, portfolio sin spoilers, garantías de originalidad, responsabilidad limitada, legislación/foro y revisión de abogado; índice de contratos alimenta M151. DELEGABLE. Pendiente QA cruzado |
| 154 — Visión del Agente | stealth/ox-alpha (Cline) | 2026-08-22 | ✅ Documentado (44/130): 4 vías de visión documentadas como alternativas (chat, MCP screen, web+Playwright, godot-mcp ⭐ fundamental); implementación MCP pendiente |
| 154 — Visión del Agente (V5 Blender) | stealth/ox-alpha (Cline) | 2026-08-22 | ✅ Vía V5 (Blender + blender-mcp) documentada al detalle en plan-actual: guía de instalación, tools, 12 capacidades, snippets bpy, protocolo de iteración; checklist 152 ítems (55 completados) |
| 154 — Visión del Agente (V4/V5 operativas) | ox-alpha (Cline) | 2026-08-24 | ✅ **V4 (godot-mcp) instalada y verificada** (Godot 4.7.2, get_godot_version + get_project_info OK) y **V5 (Blender) verificada con visión real** (esfera creada, pintada y vista por el agente). Guía maestra de conexión creada en `DOCUMENTACION/06-GUIA-DE-CONEXION-VISION.md`. Checklist 153 ítems (73 completados) |
| 52 — Partículas VFX (inicio impl.) | ox-alpha (Cline) | 2026-08-24 | 🔵 Inicio real de implementación: escena de preview de polen en Godot (`game/isla-ancestral/scenes/preview_particles.tscn` + `scripts/particles/preview_particles.gd`). **Ejecutada y validada con run_project vía godot-mcp** (logs "Polen creado OK"), depurado error de %Nodo → $Ruta, warning _delta corregido. Quedó corriendo (PID 29668). Log 145 |
| Implementación M159 Catálogo (core) | ox-alpha | Cline | 🟢 Disponible | ✅ Core implementado y validado por ox-alpha (Cline): `scripts/data/item_data.gd` + `scripts/data/item_database.gd` + `data/items/item_obj_pla_001.tres` + autoload registrado en project.godot. Compilan limpios (--check-only, 0 errores M159). 15/15 ítems checklist validados. Pendiente: 105 ítems de iconos/modelos/animaciones por categoría (requiere M45 Arte 3D + visión M154). Log 164 |
| Implementación M66 Anti-Softlock (núcleo) | ox-alpha | Cline | 🟢 Disponible | ✅ Núcleo implementado y validado (2026-08-25): SoftlockGuard autoload + IRecoverable + 6 invariantes + cofre recuperación inmutable + checkpoints atómicos (SaveWriter M59). --check-only 0 errores M66. Pendientes: hooks de disparo, lógica real NPC/misiones/puzzles/vehículos (requiere M64/M22/M24-M26/M67). Log 165 |
| Reserva M14 Inventario (análisis previo) | ox-alpha | Cline | 🔵 Reservado 2026-08-26 | Fase de documentación de hallazgos antes de implementar: colisión class_name ItemData (M159), contrato esperado por ShopManager (M39), primer ISaveProvider real (M59), patrones de skill godot-inventory-system. Hallazgos en DOCUMENTACION/14-Inventario/plan-actual/04-Codigo.md |
| Implementación M14 Inventario (iteración 1: núcleo) | ox-alpha | Cline | 🟢 Liberado — iteración 1 completa 2026-08-26 | Autoload `Inventario` + contenedores + ISaveProvider real de M59. Regresión save 13/13 OK. Siguiente: M38 Economía (desbloquea prueba end-to-end de M39/M14). Logs 168 (hallazgos) y 169 (implementación) |
| ⚠️ Hallazgo: voxel sin soporte web | ox-alpha (Cline) | 2026-08-25 | 🟡 Abierto tema `04-Voxel-Sin-Soporte-Web`: el addon `zylann.voxel` no tiene binarios `wasm32` → en el build web `main_isla.gd` falla al parsear y el gameplay (WASD) está muerto (verificado con Playwright + capturas). Desktop sin afectación. El agente del módulo voxel debe leer `Mensajes entre modelos/04-Voxel-Sin-Soporte-Web/2026-08-25_1-OX-ALPHA-hallazgo-voxel-web.md`. Logs 159-160 |
| 🟢 Ecosistema de visión para OpenCode | ox-alpha (Cline) | 2026-08-25 | Tema `05-Ecosistema-Vision-Para-OpenCode`: aclaración para agentes sin MCP (OpenCode) — los scripts de `tools/mcp/godot-mcp/scripts-reutilizables/` (cap_godot.py, qa_web.py, lanzar_preview.py, etc. + venv) son utilizables por consola sin MCP; solo V2-MCP/V4/V5 quedan exclusivas de Cline. Reglas de capturas incluidas |
